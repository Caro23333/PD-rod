...
@wp.kernel
def computeRodCenter(q: wp.array(dtype = float),
                     res: wp.array(dtype = wp.vec3)):
    i = wp.tid()
    base3 = 3 * i
    res[i] = wp.vec3(q[base3], q[base3 + 1], q[base3 + 2])

updateDamping = wp.constant(1.0)

@wp.kernel
def updateTrans(v: wp.array(dtype = float), 
                xLast: wp.array(dtype = float),
                xNext: wp.array(dtype = float),
                qGuess: wp.array(dtype = float),
                n: int, dtInv: float):
    i = wp.tid()
    dest = qGuess[i] + (xNext[i] - qGuess[i]) * updateDamping
    v[i] = (dest - xLast[i]) * dtInv
    xLast[i] = xNext[i]
...
def updateRot(w: wp.array(dtype = float),
    ...
    uGuess = wp.quaternion(qGuess[start + base4], qGuess[start + base4 + 1], qGuess[start + base4 + 2], qGuess[start + base4 + 3])
    u2 = wp.quat_slerp(uGuess, u1, updateDamping)
    u0 = wp.quaternion(uLast[base4], uLast[base4 + 1], uLast[base4 + 2], uLast[base4 + 3])
    w1 = wp.quat_inverse(u0) * u2 * 2.0 * dtInv
    w[base3] = w1[0]
    w[base3 + 1] = w1[1]
    w[base3 + 2] = w1[2]
    uLast[base4] = u1[0]
...
class Sim:
    ...
    def __init__(self, fps: int = 30, stepPerFrame: int = 2, rod: Rod = Rod(), solverIter: int = 8, wSE = -1.0, wBT = -1.0):
        self.rod = rod
        self.n = rod.n
        ...
        self.lInv = 1 / rod.l
        self.wSE = rod.E * rod.A3 * rod.l if wSE == -1.0 else wSE
        self.wBT = 4 * rod.G * rod.J3 * self.lInv if wBT == -1.0 else wBT
        ...
        self.center = wp.zeros(shape = (self.n, ), dtype = wp.vec3)
        ...
        self.hash_grid_radius = 2.0 * self.rod.radius + threshold + self.rod.length / self.n
    ...
    def addTransDBC(self, nodeIndex: int, x0: wp.vec3, v: wp.vec3, tmin = 0.0, tmax = 10.0 ** 6):
        self.transDBCx0 += [x0[0], x0[1], x0[2]]
        self.transDBCv += [v[0], v[1], v[2]]
        self.transDBCindex += [nodeIndex]
        self.transDBCtmin += [tmin]
        self.transDBCtmax += [tmax]

    def addRotDBC(self, edgeIndex: int, d0: wp.vec3, rotAxis: wp.vec3, w: float, tmin = 0.0, tmax = 10.0 ** 6):
        u0 = getRotation(d0)
        self.rotDBCu0 += [u0[0], u0[1], u0[2], u0[3]]
        self.rotDBCaxis += [rotAxis]
        self.rotDBCw += [w]
        self.rotDBCindex += [edgeIndex]
        self.rotDBCtmin += [tmin]
        self.rotDBCtmax += [tmax]
    
    def addSphere(self, x0: wp.vec3, v: wp.vec3, radius: float):
        self.spheres += [initNewSphere(x0, v, radius)]
    
    def addTable(self, p0: wp.vec3, p1: wp.vec3, p2: wp.vec3, p3: wp.vec3, reverse: bool = False):
        self.tables += [initNewTable(p0, p1, p2, p3, reverse)]
    ...
    def initBoundary(self):
        ...
        self.wp_transDBCtmin = wp.array(self.transDBCtmin, dtype = float)
        self.wp_transDBCtmax = wp.array(self.transDBCtmax, dtype = float)
        ...
        self.wp_rotDBCtmin = wp.array(self.rotDBCtmin, dtype = float)
        self.wp_rotDBCtmax = wp.array(self.rotDBCtmax, dtype = float)

        self.sphereNum = len(self.spheres)
        self.wp_spheres = wp.array(self.spheres, dtype = Sphere)
        self.tableNum = len(self.tables)
        self.wp_tables = wp.array(self.tables, dtype = Table)
    ...
    def timeStep(self, fExt = wp.zeros(), tExt = wp.zeros()):
        ...
        wp.launch(
            ...
            inputs = [
                self.wp_transDBCx, self.wp_transDBCv, 
                self.wp_transDBCtmin, self.currentTime, self.dt
            ]
        )
        wp.launch(
            advanceRotDBC, self.rotDBCNum,
            inputs = [
                self.wp_rotDBCu, self.wp_rotDBCaxis, self.wp_rotDBCw, 
                self.wp_rotDBCtmin, self.currentTime, self.dt
            ]
        )
        # initialize guess
        wp.launch(
        ...
        wp.launch(
            ...
            inputs = [
                ...
                self.wp_transDBCtmin, self.wp_transDBCtmax,
                self.wp_rotDBCtmin, self.wp_rotDBCtmax,
                self.wp_transDBCindex, self.wp_rotDBCindex, self.b, 
                self.n, self.transDBCNum, self.currentTime
            ]
        )
        wp.launch(
            computeRodCenter, dim = self.n,
            inputs = [self.q, self.center]
        )
        self.collisionCount.fill_(0)
        self.hash_grid.build(self.center, self.hash_grid_radius)
        # accumulate sphere collision potential
        wp.launch(
            sphereCollisionPotential, dim = (self.n + 1, self.sphereNum),
            inputs = [
                self.q, self.wp_spheres, 
                self.collisionCount, self.b, 
                self.rod.radius, self.currentTime
            ]
        )
        # accumulate table collision potential
        wp.launch(
            tableCollisionPotential, dim = (self.n + 1, self.tableNum),
            inputs = [
                self.q, self.wp_tables, 
                self.collisionCount, self.b, self.rod.radius
            ]
        )
        # accumulate rod collision potential
        wp.launch(
            rodCollisionPotential, dim = self.n,
            inputs = [
                self.q, self.collisionCount, 
                self.b, self.n, self.rod.radius,
                self.hash_grid.id, self.hash_grid_radius
            ]
        )
        # solve linear system
        ...
        wp.launch(
            ...
            inputs = [self.rod.v, self.rod.x, self.q, self.s, self.n, self.dtInv]
        )
        # update angular velocity
        wp.launch(
            updateRot, self.n,
            inputs = [self.rod.w, self.rod.u, self.q, self.s, self.n, self.dtInv]   
        )

    def conjugate_gradient(self, maxIter = 20, tol = 0):
        BLOCK = 2 ** 5
        ...
        wp.launch(
            ...
            inputs = [
                ...
                self.wSE, self.wBT, self.lInv, self.currentTime
            ]
        )
        self.Ax.fill_(0.0)
        wp.launch(
    ...
    def export(self, fileName, filePath, index, resolution): # not used
        vertices = []
        nodeIndex = []
        edgeIndex = []
        faces = []
